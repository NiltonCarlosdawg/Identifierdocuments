...
export async function verifyDocumentContainsIdentifier(
  ...
  try {
    const isPhysical = mimeType.startsWith("image/") || ext === ".jpg" || ext === ".jpeg" || ext === ".png";
    ...
    if (mimeType === "application/pdf" || ext === ".pdf") {
      ...
      // CORREÇÃO (Fix 5): Scans físicos (imagens) ainda não têm OCR. 
      // Permitimos o anexo marcando como verificação manual pendente.
      return { found: true, identifier, mimeType, method: "manual-verify-required", excerpt: "[Imagem: Verificação manual necessária]" };
    } else {
